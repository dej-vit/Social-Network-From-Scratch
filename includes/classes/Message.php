	...
	public function getConvos() {
		...
		foreach($convos as $username) {
			...
			$return_string .= "<a href='messages.php?u=$username'> <div class='user_found_messages'>
								 <img class='user_found_messages' style='border-radius: 5px; margin-right: 5px;  ' src='assets/images/profile_pics/" . $row['profile_pic']." '>
	...
	public function getConvosDropdown($data, $limit) {
		...
		foreach($convos as $username) {
			...
			$split = $split[0] . $dots;
		     ...
		     $user_details_query = mysqli_query($this->con, "SELECT * FROM users WHERE username = 
		     	'$username'");
             $row = mysqli_fetch_array( $user_details_query);
             $profile_pic = $row['profile_pic']; 
			...
			$return_string .= "<a href='messages.php?u=$username'> 
								<div class='user_found_messages' style='" . $style . "'>
								 <img  style='border-radius: 5px;
                                  margin-right: 5px; height:30px; float:left;' src='assets/images/profile_pics/" . $row['profile_pic']." '>
								" . $user_found_obj->getFirstAndLastName() . "
								<span class='timestamp_smaller' id='grey'> " . $latest_message_details[2] . "</span>
								<p id='grey' style='margin: 0;'>" . $latest_message_details[0] . $split . " </p>
								</div>
